...
### 3. 准备Excel数据
系统会自动从项目根目录的 `data` 文件夹中读取咖啡订单Excel文件。
请确保已运行咖啡订单监控程序生成数据文件。
...
在 `app.py` 中可以修改以下配置：
```python
EXCEL_FOLDER = os.path.join(PROJECT_ROOT, "data")  # Excel文件存放文件夹
EXCEL_PATTERN = "*咖啡订单*.xlsx"  # Excel文件匹配模式
```
